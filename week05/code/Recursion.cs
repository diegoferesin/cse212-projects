...
public static class Recursion
{
    ...
    {
        // Base case: if the current word length equals the target size, add it to results
        if (word.Length == size)
        {
            results.Add(word);
            return;
        ...
        for (int i = 0; i < letters.Length; i++)
        {
            ...
            // Create a new letters string without the used letter
            string remainingLetters = letters.Substring(0, i) + letters.Substring(i + 1);
            
            // Recursively generate permutations with the remaining letters
    ...
    public static void SolveMaze(List<string> results, Maze maze, int x = 0, int y = 0, List<ValueTuple<int, int>>? currPath = null)
    {
        ...
        // Base case: if we're at the end, add the current path to results
        if (maze.IsEnd(x, y))
        {
